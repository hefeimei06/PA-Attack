...
class GuidanceImages(Dataset):
    def __init__(self, root, lvlm_model):
        # 移除随机种子设定，因为我们不再需要随机采样
        self.lvlm_model = lvlm_model
        
        # 修改 1: 读取 .png 文件 (我们刚刚保存的格式)
        # 修改 2: 读取目录下所有文件，不再需要 random.sample，因为文件夹里正好是3000张
    ...
    def __getitem__(self, idx):
        image = Image.open(self.paths[idx])
        image.load()
        # 确保转为 RGB，防止个别图片保存时异常
        if image.mode != 'RGB':
            image = image.convert('RGB')
            
        target_images = [[image]]
        
        # 核心: 保持原有的预处理逻辑，确保与 LVLM 对齐
        target_images = self.lvlm_model._prepare_images(target_images)
        target_images = target_images.to(self.lvlm_model.device)[0]
...
def main():
    ...
    guidance_path = "scienceqa_guidance"
    n_prototypes = 20

    dataset = GuidanceImages(root=guidance_path, lvlm_model=lvlm_model)
    ...
    os.makedirs("prototypes_llava_tokens_bestpca_scienceqa", exist_ok=True)

    torch.save(proto_tokens, "prototypes_llava_tokens_bestpca_scienceqa/prototypes_tokens_3000_20_1024.pt")
