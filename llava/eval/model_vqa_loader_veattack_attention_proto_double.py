...
class ClipVisionModel(torch.nn.Module):
    ...
    def forward(self, vision, output_normalize, tokens=False, attention=False):
        if not tokens:
            ...
            if attention == True:
                feature, tokens, attentions = self.model(self.normalize(vision), output_attentions=True)
                if output_normalize:
                    feature = F.normalize(feature, dim=-1)
                    tokens = F.normalize(tokens, dim=-1)
                return feature, tokens, attentions
            else:
...
class ComputeLossWrapper:
    ...
    def __call__(self, embedding, tokens):
        return compute_loss(embedding=embedding, embedding_orig=self.embedding_orig, tokens_mask=self.tokens_mask,
                            tokens=tokens, tokens_orig=self.tokens_orig, target_proto_tokens=self.target_proto_tokens,
...
def get_of_args(pretrained_rob_path=None):
    ...
    model_args['checkpoint_path'] = '/home/hefei/AttackVLM/RobustVLM-Attack/ckpt/checkpoint.pt'
    ...
    for (input_ids, image_tensor, index) in tqdm(data_loader, total=len(data_loader)):
        ...
        if args.eval_model == 'LLAVA':
            ...
            if args.attack == 'veattack':
                proto_tokens = torch.load("prototypes_llava_tokens_bestpca/prototypes_tokens_3000_20_1024.pt").to(device='cuda', non_blocking=True)

                args.output_normalize = False
                image_tensor = image_tensor.to(device='cuda', non_blocking=True)

                with torch.no_grad():
                    embedding_orig, tokens_orig, attentions = clip_model_vision(vision=image_tensor,
                                                                                output_normalize=args.output_normalize,
                                                                                tokens=True, attention=True)
                    cls2patch = attentions[12][:, :, 0, 1:].mean(dim=1)
                    # cls2patch = torch.stack(attentions[11: 14], dim=0).mean(dim=0)[:, :, 0, 1:].mean(dim=1)
                    tokens_mask = F.softmax(20 * cls2patch, dim=-1)
                ...
                # target_proto_embeddings = proto_embedding[min_idx].unsqueeze(0)
                target_proto_tokens = proto_tokens[min_idx].unsqueeze(0)

                loss_inner_wrapper = ComputeLossWrapper(embedding_orig, tokens_orig, target_proto_tokens, tokens_mask, 'none')
                args.norm = 'linf'
                ...
                with torch.no_grad():
                    _, _, attentions = clip_model_vision(vision=image_tensor,
                                                        output_normalize=args.output_normalize,
                                                        tokens=True, attention=True)
                    cls2patch = attentions[12][:, :, 0, 1:].mean(dim=1)
                    # cls2patch = torch.stack(attentions[11: 14], dim=0).mean(dim=0)[:, :, 0, 1:].mean(dim=1)
            ...
            if args.attack == 'veattack':
                ...
                proto_tokens = torch.load("prototypes_of_tokens_bestpca/prototypes_tokens_3000_20_1024.pt").to(device='cuda', non_blocking=True)

                args.output_normalize = False
                batch_images = batch_images.to(device='cuda', non_blocking=True)

                with torch.no_grad():
                    embedding_orig, tokens_orig, attentions = clip_model_vision(vision=batch_images,
                                                                                output_normalize=args.output_normalize,
                                                                                tokens=True, attention=True)
                    cls2patch = attentions[12][:, :, 0, 1:].mean(dim=1)
                    # cls2patch = torch.stack(attentions[11: 14], dim=0).mean(dim=0)[:, :, 0, 1:].mean(dim=1)
                ...
                with torch.no_grad():
                    _, _, attentions = clip_model_vision(vision=batch_images,
                                                        output_normalize=args.output_normalize,
                                                        tokens=True, attention=True)
                    cls2patch = attentions[12][:, :, 0, 1:].mean(dim=1)
                    # cls2patch = torch.stack(attentions[11: 14], dim=0).mean(dim=0)[:, :, 0, 1:].mean(dim=1)
