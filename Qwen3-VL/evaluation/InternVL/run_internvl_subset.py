...
OUTPUT_FILE = "results/internvl_paattack_results_sub500_100_200.jsonl"
...
def main():
    ...
    if SUBSET_NUM is not None and SUBSET_NUM > 0:
        if SUBSET_NUM < len(df):
            print(f"✂️  Subset mode enabled: only using first {SUBSET_NUM} samples for testing.")
            df = df.head(SUBSET_NUM)
        else:
            print(f"⚠️ Subset  ({SUBSET_NUM}) is larger than total samples ({len(df)}), using all data instead.")
    else:
        print("✅ Running on the full dataset (no subset slicing).")

    os.makedirs(os.path.dirname(OUTPUT_FILE), exist_ok=True)
    if os.path.exists(OUTPUT_FILE):
        os.remove(OUTPUT_FILE)

    print("▶️ Start evaluation (InternVL Chat Mode)...")
