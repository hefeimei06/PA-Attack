...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, visual_feature: bool = False, **kwargs) -> torch.Tensor:
        """
        Args:
...
def pgd_train(
    ...
    for i in range(iterations):
        ...
        with torch.enable_grad():
            tokens = forward(data_clean + perturbation, grid_thw=image_grid_thw, visual_feature=True)
            loss = loss_fn(tokens)
            if verbose:
...
def compute_loss(feature, features_orig, tokens_mask, target_proto_tokens, reduction='mean'):
    ...
    loss_target = cosine_similarity_loss(out=feature, targets=target_proto_tokens, reduction=reduction)
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None):
        ...
        proto_tokens = torch.load("Prototype/prototypes_qwen3/prototypes_global_3000_20_1024.pt").to("cuda")

        with torch.no_grad():
            tokens_orig = self.visual(pixel_values, grid_thw=image_grid_thw, visual_feature=True)
        ...
        token_sims = torch.matmul(tokens_norm, proto_tokens_norm.T)
        emb_similarity = token_sims.mean(dim=0)
        min_sim, min_idx = torch.min(emb_similarity, dim=0)
        ...
        pixel_values = pgd_train(
            forward=self.visual,
            image_grid_thw=image_grid_thw,
            loss_fn=loss_inner_wrapper,
            data_clean=pixel_values,
            norm=norm,
            eps=eps,
            iterations=100,
            stepsize=stepsize_adv,
            perturbation=torch.zeros_like(pixel_values).uniform_(-eps, eps).requires_grad_(True),
            mode='max',
            verbose=False
        )
